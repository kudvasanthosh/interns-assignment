...
async function getAll(queryParam) {
    ...
    if (queryParam.pricefrom && queryParam.priceto) {
        matchQuery.productPrice = { $gte: queryParam.pricefrom, $lte: queryParam.priceto }
    }

    let productItems = await Productdb.find(matchQuery).limit(numRecords).skip(parseInt(skip)).populate('productCategory').select({ productName: 1, productPrice: 1 });
