...
async function create(userParam) {
    // validate
    //  if (await Product.findOne({ productname: userParam.productname })) {
    //      return { "status":400,"message": `product  ${userParam.productname} is already added` }
    //  }
    //   if(await Category.find({categoryname:userParam.productcategory}))
    //   {
    //       return {"status":400,"message":`product category ${userParam.productcategory} is not a category`}
    //   }
try{
